...
Chanslator 是一个本地运行的中英双向文档翻译与审校工具。它支持拖入 DOC、DOCX 和 PDF，自动提取文本、调用 OpenAI 或 Anthropic 兼容接口翻译，并在左右对照界面中逐段审校，最终导出 DOCX 译文。
...
3. 程序会自动打开 Chanslator 桌面窗口。
4. 关闭窗口时，本地服务会一同退出。

### 方式二：PowerShell 启动

```powershell
Set-Location D:\github\translator
.\start.ps1
```
...
- 端口被占用：关闭已有 Chanslator 进程，或修改 `.env` 中的 `APP_PORT`。

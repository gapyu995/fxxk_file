...
Translator 是一个本地运行的中英双向文档翻译与审校工具。它支持拖入 DOC、DOCX 和 PDF，自动提取文本、调用 OpenAI 或 Anthropic 兼容接口翻译，并在左右对照界面中逐段审校，最终导出 DOCX 译文。
...
- 端口被占用：关闭已有 Translator 进程，或修改 `.env` 中的 `APP_PORT`。
